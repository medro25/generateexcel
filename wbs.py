...
time.sleep(1)
if (len(project_obj)>9):
    session.findById("wnd[0]/usr/ctxtCN_PSPNR-LOW").text = project_obj
    time.sleep(1)
else:
    input_field = session.findById("wnd[0]/usr/ctxtCN_PROJN-LOW")
    input_field.text = project_obj
    input_field.caretPosition = len(project_obj)
time.sleep(2)
...
time.sleep(1)
session.findById("wnd[1]/usr/btn%PC").Press()
time.sleep(1)
